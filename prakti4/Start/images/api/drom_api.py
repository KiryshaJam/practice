...
class DromAPI(BaseAPI):
    BASE_URL = "https://api.drom.ru/v1"
    ...
    def get_cars(self, **kwargs) -> List[Dict[str, Any]]:
        """Получение списка автомобилей с Drom.ru"""
        params = {
        ...
        response = self._make_request(f"{self.BASE_URL}/auto", params=params)
        return [self._normalize_car_data(car) for car in response.get('items', [])]

    def get_car_details(self, car_id: str) -> Dict[str, Any]:
        """Получение детальной информации об автомобиле"""
        response = self._make_request(f"{self.BASE_URL}/auto/{car_id}")
